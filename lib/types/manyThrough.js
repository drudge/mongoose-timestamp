...
module.exports.loadType = function (mongoose) {
  function ManyThrough (docs, parentDoc, path, hydrate) {
    ...
    if (!this.referredThruType) throw new Error("thru model is missing. Make sure you're requiring " + type.options.thru + " somewhere in your code.");
  ...
  ManyThrough.prototype.add = function (toAdd, thruAttrs, fn) {
    var attr;
    if (arguments.callee.length === 2) {
      if (typeof thruAttrs === "function") {
        fn = thruAttrs;
        thruAttrs = {};
      }
    }
    if (toAdd instanceof this.memberType) { // A fully resolved mongoose Document instance
      thruAttrs[this.parentReferredAs] = new DBRef(this.parent._schema._collection, this.parent._id);
      thruAttrs[this.targetReferredAs] = new DBRef(toAdd._schema._collection, toAdd._id);
      this.referredThruType.create(thruAttrs, function (err, created) {
        if (fn) fn(err);
